...
const validateDocuments = (documents) => {
    ...
    const invalidDocument = documents.find((document) => {
        if (typeof document === "string") {
            return !documentTypes.includes(document);
        }

        if (document && typeof document === "object") {
            return !document.type || !documentTypes.includes(document.type);
        }
...
export const updateUser = async (id, data) => {
    ...
    if (data.email !== undefined) {
        ...
        data.email = validateStringField(
            data.email, 
            "email", 
            { 
                required: true, 
    ...
    // Si vienen documentos nuevos, se agregan a los existentes.
    if (payload.documents !== undefined) {
        payload.documents = validateDocuments(payload.documents);
...
export const addDocument = async (id,file, type) => {
    ...
    return UserRepository.updateUser ( id , {documents});
}